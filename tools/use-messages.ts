...
const obj2msg = (obj: { role: string; content: string }): BaseChatMessage =>
  obj.role === "user"
    ? new HumanChatMessage(obj.content)
    : new AIChatMessage(obj.content);
...
const initialMessages = (hasApiKey: boolean): BaseChatMessage[] =>
  hasApiKey
    ...
    : [new AIChatMessage(instructionMessage)];
...
function useMessages() {
  ...
  useEffect(() => {
    if (settings.countTokens) {
      ...
      getTokenInfo([new SystemChatMessage(systemMessage), ...messages])
        .then(setTokenInfo)
        .catch((err: any) =>
          console.warn("Unable to count tokens in messages", err.message)
        );
    } else {
  ...
  return {
    ...
    removeMessage(message: BaseChatMessage) {
      setMessages(
        (messages || initialMessages(hasApiKey)).filter((m) => m !== message)
      );
    },
  };
}
